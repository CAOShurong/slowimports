...
- `--apply` rewrites a `.py` file: imports that `--advice` would list are
  moved into the functions that use them. `--apply-dry-run` prints a
  unified diff. Combined `import a, b` lines are split when only some
  names move. `--apply` refuses site-packages / stdlib paths and honours
  `--min-saving`. Commented and one-line-function cases are left alone.
