...
ADAPTER_FILES = ("AGENTS.md", "CLAUDE.md", "GEMINI.md")
...
def git_fingerprint(project: Path, root: Path) -> tuple[str, str]:
    ...
    for relative in candidates:
        if is_memory_relative(relative, scope):
            continue
        path = root / relative
...
def filesystem_manifest_fingerprint(project: Path) -> tuple[str, str]:
    ...
    for directory, dirnames, filenames in os.walk(project):
        ...
        for filename in sorted(filenames):
            ...
            if is_memory_relative(relative):
                continue
...
def git_portability_status(project: Path) -> tuple[list[str], list[str]]:
    ...
    portable_files = list(REQUIRED_FILES) + [PROJECT_TOOL]
...
def inspect_project(project: Path) -> tuple[list[str], list[str], dict[str, str]]:
    ...
    allowed_statuses = {"not_started", "active", "blocked", "paused", "complete"}
...
def ensure_export_fresh(project: Path, handoff: str) -> None:
    ...
    if (
        ...
        raise ValueError(
            ...
            "workspace_fingerprint and context_fingerprint after verifying the project."
        )
...
def command_export(args: argparse.Namespace) -> int:
    ...
    manifest = [
        f"| `{relative}` | `{file_sha256(project / relative)}` |"
        for relative in EXPORT_FILES
    ]
